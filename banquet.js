(() => {
  ...
  function render() {
    ...
    box.innerHTML = cs.map(c => {
      ...
      return `<section class="banquet-group" id="banquet-${esc(c.id)}" data-bgroup="${esc(c.id)}" data-banquet-category-section="${esc(c.id)}"><h2 class="banquet-group__title">${esc(c.name)}</h2><div class="banquet-list">${groupItems.map(i => `<button type="button" class="banquet-card" data-bitem="${esc(i.id)}"><span><span class="banquet-card__name">${esc(text(i,'name'))}</span>${text(i,'summary') ? `<span class="banquet-card__summary">${esc(text(i,'summary'))}</span>` : ''}<span class="banquet-card__price">${esc(price(i.price))}</span></span><span class="material-symbols-outlined banquet-card__chevron" aria-hidden="true"><svg aria-hidden="true"><use href="assets/icons.svg#chevron_right"></use></svg></span></button>`).join('')}</div></section>`;
    }).join('');
  }
