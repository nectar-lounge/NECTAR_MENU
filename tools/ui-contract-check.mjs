...
ok((html.match(/<section class="hero"/g)||[]).length===2,'Menu and Banquet states must use the same hero component');
ok(!html.includes('class="banquet-hero"'),'legacy standalone banquet hero/card must not return');
...
ok(app.includes("const banquetActive = state.section === 'banquet'"),'selector active state must track Banquet');
ok(app.includes("? section === 'menu' || section === 'banquet'"),'bottom Menu destination must stay active for Banquet');
